...
class UserService:
    ...
    def get_client_structure(self, vkorg: str) -> ClientStructure:
        ...
        if isinstance(structure_raw, Sequence) and not isinstance(
            structure_raw, (str, bytes)
        ):
        ...
        for entry in entries:
            ...
            try:
                buyers.append(
                    Buyer(
                        id=str(
                            require_value(data, ("ID", "KUNRG", "KUNNR"), "RG_TAB.id")
                        ),
                        is_default=parse_bool_flag(
                            first_value(data, ("DEFAULT", "IS_DEFAULT", "DEF"))
                        ),
                        short_name=first_value(
                            data, ("SORTL", "SHORT_NAME", "SHORTNAME")
                        ),
        ...
        for entry in entries:
            ...
            try:
                addresses.append(
                    DeliveryAddress(
                        id=str(
                            require_value(data, ("ID", "KUNWE", "KUNNR"), "ZA_TAB.id")
                        ),
                        is_default=parse_bool_flag(
                            first_value(data, ("DEFAULT", "IS_DEFAULT", "DEF"))
                        ),
                ...
                raise ArmtekResponseFormatError(
                    "Invalid delivery address entry format"
                ) from exc
        return addresses
            ...
            try:
                points.append(
                    PickupPoint(
                        id=str(
                            require_value(
                                data, ("ID", "EXW_ID", "POINT_ID"), "EXW_TAB.id"
                            )
                        ),
                ...
                raise ArmtekResponseFormatError(
                    "Invalid pickup point entry format"
                ) from exc
        return points
                ...
                contracts.append(
                    Contract(
                        ...
                        date=parse_datetime_value(
                            first_value(data, ("DATE", "DATUM", "ERDAT"))
                        ),
                        valid_to=parse_datetime_value(
                            first_value(data, ("VALID_TO", "VALDT", "GUELTIGBIS"))
                        ),
