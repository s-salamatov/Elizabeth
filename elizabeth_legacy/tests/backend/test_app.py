...
class DummyArmtekService:
    def __init__(
        ...
        context: ArmtekSearchContext | None = None,
    ):
        self.items = items or []
        self.error = error
        self.calls = []
        self.search_context = context or ArmtekSearchContext(
            vkorg="4000", kunnr_rg="123"
        )
...
def test_health_endpoint():
    app = create_app(armtek_service=DummyArmtekService())
...
def test_characteristics_endpoints_flow():
    ...
    invalid = client.post(
        "/api/armtek/characteristics", json={"token": "", "artid": "123"}
    )
    assert invalid.status_code == 400
    invalid_type = client.post(
        "/api/armtek/characteristics", json={"token": token, "artid": 123}
    )
