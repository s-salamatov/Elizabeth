...
def test_get_vkorg_list_http_error():
    service = UserService(
        DummyHttpClient({("get", "/api/ws_user/getUserVkorgList"): ArmtekHttpError("network")})
    )
...
def test_get_client_structure_success():
    ...
    service = UserService(DummyHttpClient({("post", "/api/ws_user/getUserInfo"): response}))
...
def test_get_client_structure_http_error():
    service = UserService(DummyHttpClient({("post", "/api/ws_user/getUserInfo"): ArmtekHttpError("boom")}))
